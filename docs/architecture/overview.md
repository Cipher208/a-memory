...
Agents see the five primitives (`think` / `dream` / `forget` / `evolve` / `project`) by default; `ARIEL_EXPOSE=primitives,wiki` adds the four wiki tools, and `ARIEL_EXPOSE=all` restores the full 34-tool surface.
