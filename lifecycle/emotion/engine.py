...
class EmotionEngine:
    ...
    def _compile(self):
        ...
        self.emoji_regex = self._build_regex(
            [f"(?P<e_{cat.replace(' ', '_')}>{'|'.join(re.escape(i) for i in icons)})" 
             for cat, icons in self.config.emojis.items() if icons]
        )
    ...
    def _add_result(self, results: dict[str, EmotionResult], emotion: str, score: float, source: str, match_text: str):
    ...
    def _match_phrases(self, text: str, results: dict[str, EmotionResult]):
    ...
    def _match_markers(self, text: str, results: dict[str, EmotionResult]):
    ...
    def _match_emojis(self, text: str, results: dict[str, EmotionResult]):
