...
async def test_fire_mixed_handlers(registry):
    ...
    class Mock:
        ...
        def h1(self, ctx): results.append('s')
        @registry.mark('m', layer='user')
        async def h2(self, ctx): results.append('a')
...
async def test_fire_fail_safe(registry):
    class Mock:
        @registry.mark('m')
        def h1(self, ctx): raise ValueError('fail')
        @registry.mark('m')
        def h2(self, ctx): return 'ok'
    registry.register_instance(Mock())
...
async def test_fire_instance_binding(registry):
    class Mock:
        def __init__(self): self.val = 'bound'
        @registry.mark('test')
        def h(self, ctx): return self.val
    registry.register_instance(Mock())
