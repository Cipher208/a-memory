...
|---|---|---|---|
| **Hermes** | `~/.hermes/state.db` → `messages(id, session_id, role, content, timestamp)`; filter `role IN ('user','assistant')` | `~/.hermes/hooks/ariel-autohooks/` — `HOOK.yaml` declares `session:start` + `session:end`; `handler.py` runs inject (start) / dispatch (end). Inject output lands in `~/.hermes/context/ariel-inject.md` | live |
| **MiMoCode** | `~/.local/share/mimocode/mimocode.db` → `part(rowid, message_id, data JSON)`; role via subquery on `message.data`, text via `json_extract(data,'$.text')`, filter `json_extract(data,'$.type')='text'` | `~/.config/mimocode/hooks/ariel-inject.ts` — `export default {"session.start": …, "session.end": …}` (contract per `rules-discovery.ts`); inject sets `output.context` | live (session.end event unverified upstream) |
| **CowAgent** (persona Eli) | `~/cow/memory/long-term/index.db` → `messages(id, session_id, seq, role, content, created_at)` | daemon live; inject point TBD (platform plugins surface) | daemon live |
