...
| Tool | Description |
|------|-------------|
| `memory_graph_add` | Add a node (`content`, `node_type`, `tags`); type maps to domain hooks (error/decision/personality/emotion). Social types (`person`, `organization`) dedup per user — re-add returns the existing node. Optional `relates_to` (node_id) + `relation` (`knows`, `works_with`, `family_of`, `friend_of`, `met`, `mentions`) create an edge in the same call. E17a/B1.7 producer: `action="causal"` instead records an action→outcome causal link (`content`=action, `outcome`, `relation` ∈ `led_to`/`caused`/`blocked`, `strength` = causal weight; idempotent action/outcome nodes) — read back via the temporal graph's causal-chain query. |
...
| `memory_disclose` | Disclosure triggers (E11) — the recall-side pair of `memory_watch`: operator rules of the form «when the text contains X, surface Y». `action="add"` (`name`, `trigger_keywords` list — OR semantics, case-insensitive substring match, `content`); `"list" | "enable" | "disable" | "delete"` by `rule_id`. Rules surface as a `triggered` block (score 0.95) in `recall_protocol` and the session-start inject. Feature-private `disclosure_rules` table (lazy `_ensure_table`, no migration). Write tier. Tool count 62 → 63. |
| `memory_proposals` | Review surface for staged mutations (C1.11): `action="list"` returns pending proposals for `user_id`; `action="decide"` applies (`approve=true`) or rejects (`approve=false`) one proposal by `proposal_id`; `action="revert"` undoes an **applied** proposal with exact provenance (core_write / archive / wiki_write — E17b removes the created page); `action="revert_transition"` (E17c/C1.13) undoes one consolidation promotion by its `memory_transitions` id — removes the promoted L4 fact, the source episode stays; `action="propose"` (E17b producer) stages a deliberate mutation from the agent side — `kind='wiki_write'` (`title`, `content`, `wiki_type?`) or `'core_write'` (`key`, `value`, `importance`). Apply executes the exact write the direct path would have; every decision is audit-logged. Pending proposals also surface in the session-start inject. |
...
| `memory_counterfactual` | Counterfactual memory (D1.20): "what could have been" notes branching from a real anchor (fact key, decision, episode ref). `action="save"` (anchor+premise+projection) stores the alternative; `action="list"` reads by anchor. Reflective material — explicit save, list by anchor. Tool count 48 → 49. |
| `memory_recap` | Session continuity recap (D1.2): the /new recovery pack. One-shot block list for a fresh session — last closed session (summary, topics, state deltas) → pending work (scratchpad notes, diff gaps, staged proposals) → dream markers → day digest, all within `budget` tokens. Call instead of re-reading raw history (~2K tokens vs 50K); the same engine drives the `autohooks recap` CLI. Tool count 49 → 50. |
| `memory_steering` | Steering hints (D1.3): routes common intents to the best ariel tools. Deterministic route table (8 routes: recap / recall protocol / scratchpad vs remember / skills / quality feedback / proposals / stats) + keyword intent match (RU/EN), max 3 hints; empty query returns the full table (boot-time system-prompt appendix). Advisory only — ariel publishes routes, the harness decides. Tool count 50 → 51. |
| `memory_compress` | Tool-output compression (D1.4): shrink bulky text before re-injecting it into context. `mode="log"` — build/test logs → errors only (+ first header line, consecutive-duplicate collapse, `max_lines` cap); `mode="code"` — Python → skeleton via ast (signatures kept, bodies dropped as `...`; non-Python falls back to log); `"auto"` — tries code first. Returns `{mode, original_lines, kept_lines, text}`. Deterministic, no LLM. Tool count 51 → 52. |
| `memory_fact_blame` | Provenance / fact blame (D1.6): who wrote this fact, when, and why — the evidence trail for debugging hallucinations. Lookup by `entry_id` or `key`; rides the existing `core_memory.source` column (no migration): provenance ∈ {`user_explicit`, `staging_promotion`, `episode_promotion`, legacy `manual`}. Returns the entry + `importance_audit` history (chunk_id = entry_id) + `audit_log` events (target_id = entry_id). Tool count 52 → 53. |
| `memory_query` | Memory Query DSL (D1.7): structured analytics without raw SQL. Whitelisted filters → parameterized SQL over `core_memory` or `episodes` (`importance_min/max`, `key_like`, `content_like`, `created_since/until`, episodes-only `tag` + E10 faceted `tags` — `dimension:value` entries, same-dimension values OR'd, dimensions AND'd via `json_each`; a plain tag forms its own dimension — `limit` ≤ 200). Read-only, no injection surface. Tool count 53 → 54. |
