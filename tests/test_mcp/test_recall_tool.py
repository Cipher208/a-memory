...
def test_registered_and_tier():
    ...
    assert (
        len(_register_tools) == 63
    )  # 41 + recall + wiki_read + skill_promote + smart_context + reflect + scratchpad + quality + counterfactual + recap + steering + compress + blame + query + typed + rules + history + branch + stash + procedure + diagnose + heal + disclose
