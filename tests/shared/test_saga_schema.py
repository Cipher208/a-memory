...
def test_saga_step_state_validation():
    step = SagaStepState(name="test_step", status=SagaStatus.PENDING, result={"foo": "bar"})
...
def test_saga_state_validation():
    ...
    state = SagaState(
        ...
        name="test_saga",
        context={"user_id": 1},
        steps=[SagaStepState(name="step1", status=SagaStatus.COMPLETED, result={"ok": True})],
...
def test_saga_state_serialization():
    state = SagaState(saga_id="deadbeef", name="test", context={}, steps=[], current_step_index=0, status=SagaStatus.PENDING, started_at=time.time())
