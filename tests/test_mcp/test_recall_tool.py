...
def test_registered_and_tier():
    ...
    assert (
        len(_register_tools) == 56
    )  # 41 + recall + wiki_read + skill_promote + smart_context + reflect + scratchpad + quality + counterfactual + recap + steering + compress + blame + query + typed + rules
