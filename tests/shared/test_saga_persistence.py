...
def test_save_and_load(temp_saga_store):
    state = SagaState(saga_id="test_1", name="test_saga", status=SagaStatus.RUNNING, started_at=12345.67)
...
def test_list_all(temp_saga_store):
    sagas = [SagaState(saga_id=f"saga_{i}", name=f"name_{i}") for i in range(3)]
    for s in sagas:
